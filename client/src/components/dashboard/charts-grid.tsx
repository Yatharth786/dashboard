  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        if (selectedSource === "both" || appliedFilters.table === "both") {
          const [
            flipkartRes,
            amazonRes,
            flipkartCatRes,
            ...
            fetch(`${BASE_URL}/top?table=flipkart&n=10${queryString}`),
            fetch(`${BASE_URL}/top?table=amazon_reviews&n=10${queryString}`),
          ...
          setSentiments(sentimentJson || []);
          setRapidApiProducts(rapidApiJson.data || []);
        } else if (selectedSource === "amazon_reviews" || appliedFilters.table === "amazon_reviews") {
          const [productsRes, categoriesRes, ratingsRes, sentimentRes, rapidApiRes] =
            await Promise.all([
              fetch(`${BASE_URL}/top?table=amazon_reviews&n=10${queryString}`),
              fetch(`${BASE_URL}/rapidapi_amazon_products/categories?${queryParams}`),
    ...
    fetchAll();
  }, [selectedSource, filterVersion, appliedFilters]); // ✅ Re-fetch when filters change
