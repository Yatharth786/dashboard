...
interface CompetitorProduct {
  ...
  product_star_rating_numeric: number | null;
...
interface ComparisonMetrics {
  ...
  badges: {
    seller_best_seller: boolean;
    competitor_best_seller: boolean;
    seller_amazon_choice: boolean;
    competitor_amazon_choice: boolean;
    seller_prime: boolean;
    competitor_prime: boolean;
  };
...
export default function KeywordTracker() {
  ...
  const [competitorComparisons, setCompetitorComparisons] = useState<ProductComparison[]>([]);
  ...
  const getComparisonBadge = (comparison: ProductComparison) => {
    const { competitive_advantages, competitive_disadvantages } = comparison.comparison_metrics;
    
    if (competitive_advantages.length > competitive_disadvantages.length) {
      return { text: "WINNING", color: "bg-green-500 text-white", icon: <TrendingUp className="h-3 w-3" /> };
    } else if (competitive_disadvantages.length > competitive_advantages.length) {
      return { text: "LOSING", color: "bg-red-500 text-white", icon: <TrendingDown className="h-3 w-3" /> };
    } else {
      return { text: "COMPETITIVE", color: "bg-yellow-500 text-white", icon: <Zap className="h-3 w-3" /> };
    }
  };

  // Fetch products WITH competitor comparison
  const handleFetchProductsWithComparison = async () => {
    // Check if user has reached limit
    if (userId && !canTrack) {
      setShowUpgradeModal(true);
      showToast(
    ...
    try {
      const url = `${API_BASE_URL}/keyword_tracker/fetch_and_compare/${sellerId}?country=${country}&page=${page}&user_email=${encodeURIComponent(userEmail)}${userId ? `&user_id=${userId}` : ''}`;
      ...
      // Set competitor comparisons
      setCompetitorComparisons(data.comparisons || []);
      ...
      showToast(
        "Success!", 
        `Found ${data.total_products} products and ${data.total_comparisons} competitor comparisons!`, 
        "success"
      );
  ...
  // Fetch rank history for a product
  const fetchRankHistory = async (productId: number) => {
    setLoadingRanks(true);
    try {
      const response = await fetch(
      ...
      <div className="lg:ml-64 transition-all min-h-screen">
        ...
        <header className="bg-white/70 backdrop-blur-md border-b border-slate-200 px-6 py-4 sticky top-0 z-10">
          <div className="flex items-center gap-3">
            <Target className="h-8 w-8 text-purple-600" />
            <div>
              <h2 className="text-2xl font-semibold text-slate-800">Keyword Rank Tracker with Competitor Analysis</h2>
              <p className="text-sm text-slate-500">Monitor your Amazon product rankings and compare with competitors</p>
            </div>
          </div>
        </header>
        ...
        <div className="p-6">
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="text-center space-y-4">
              <h1 className="text-4xl font-bold bg-gradient-to-r from-purple-500 via-pink-400 to-red-400 text-transparent bg-clip-text">
                Track Your Keyword Rankings & Beat Competitors
              </h1>
              <p className="text-slate-500 text-lg">
                Monitor how your products rank for important search keywords and see how you compare with similar competitors
              </p>
            ...
            {/* Step 1: Fetch Products with Competitor Comparison */}
            <Card className="shadow-sm border border-slate-200 rounded-2xl overflow-hidden backdrop-blur-md bg-white/80">
              <CardHeader>
                <CardTitle className="text-lg font-semibold text-slate-700 flex items-center gap-2">
                  <Package className="h-5 w-5 text-blue-600" />
                  Step 1: Fetch Your Products & Find Competitors
                </CardTitle>
                <CardDescription className="text-slate-500">
                  Enter your Amazon Seller ID to load your products with reviews and automatically compare with similar competitor products
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Button 
                  onClick={handleFetchProductsWithComparison}
                  disabled={loading || !userEmail || (!!userId && !canTrack)}
                  className={`w-full ${
                    userId && !canTrack
                      ? 'bg-slate-300 cursor-not-allowed'
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <Search className="h-4 w-4 mr-2" />
                      Fetch Products & Compare with Competitors
                    </>
                  )}
                </Button>
              </CardContent>
            ...
            {competitorComparisons.length > 0 && (
              <Card className="shadow-lg border-2 border-purple-300 rounded-2xl overflow-hidden backdrop-blur-md bg-gradient-to-br from-white via-purple-50 to-pink-50">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription className="text-slate-600 mt-2">
                        Compare your products with similar competitors in the market
                      </CardDescription>
                    </div>
                    <Badge className="bg-purple-600 text-white text-lg px-4 py-2">
                      {competitorComparisons.length} Comparisons
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-white p-4 rounded-xl border-2 border-green-200 shadow-sm">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm text-slate-600">Competitive Advantages</p>
                          <p className="text-2xl font-bold text-green-600">
                            {competitorComparisons.reduce((sum, c) => 
                              sum + c.comparison_metrics.competitive_advantages.length, 0
                            )}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="bg-white p-4 rounded-xl border-2 border-red-200 shadow-sm">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm text-slate-600">Areas to Improve</p>
                          <p className="text-2xl font-bold text-red-600">
                            {competitorComparisons.reduce((sum, c) => 
                              sum + c.comparison_metrics.competitive_disadvantages.length, 0
                            )}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="bg-white p-4 rounded-xl border-2 border-blue-200 shadow-sm">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm text-slate-600">Competitors Found</p>
                          <p className="text-2xl font-bold text-blue-600">
                            {new Set(competitorComparisons.map(c => c.competitor_product.asin)).size}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="space-y-6">
                    {competitorComparisons.map((comparison, index) => {
                      const badge = getComparisonBadge(comparison);
                      const metrics = comparison.comparison_metrics;
                      
                      return (
                        <div key={index} className="bg-white rounded-xl border-2 border-slate-200 shadow-md overflow-hidden">
                          {/* Header */}
                          <div className="bg-gradient-to-r from-slate-100 to-purple-100 px-6 py-4 border-b-2 border-slate-200">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-3">
                                <Badge className={`${badge.color} flex items-center gap-1 px-3 py-1`}>
                                  {badge.icon}
                                  {badge.text}
                                </Badge>
                                <span className="text-sm text-slate-600">
                                  Similarity: {(metrics.similarity_score * 100).toFixed(0)}%
                                </span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="p-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              ...
                              <div className="space-y-4">
                                <div className="flex items-center gap-2 mb-3">
                                  ...
                                  <h4 className="font-bold text-slate-800">Your Product</h4>
                                </div>
                                
                                <div className="flex gap-4">
                                  {comparison.seller_product.photo && (
                                    <img 
                                      src={comparison.seller_product.photo} 
                                      alt="Your product"
                                      className="w-24 h-24 object-cover rounded-lg border-2 border-blue-300"
                                    />
                                  )}
                                  <div className="flex-1">
                                    <p className="font-semibold text-slate-900 text-sm leading-tight mb-2">
                                      {comparison.seller_product.title}
                                    </p>
                                    <p className="text-xs text-slate-500">ASIN: {comparison.seller_product.asin}</p>
                                  </div>
                                </div>
                                ...
                                <div className="space-y-2 bg-blue-50 p-3 rounded-lg">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-bold text-blue-700">
                                      ₹{metrics.price_comparison.seller_price?.toFixed(2) || 'N/A'}
                                    </span>
                                  </div>
                                  <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-1">
                                      ...
                                      <span className="font-bold text-blue-700">
                                        {metrics.rating_comparison.seller_rating?.toFixed(1) || 'N/A'}
                                      </span>
                                    </div>
                                  </div>
                                  <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-bold text-blue-700">
                                      {metrics.review_count_comparison.seller_reviews?.toLocaleString() || 'N/A'}
                                    </span>
                                  </div>
                                  
                                  {/* Badges */}
                                  <div className="flex flex-wrap gap-1 pt-2">
                                    {metrics.badges.seller_best_seller && (
                                      <Badge className="bg-orange-100 text-orange-800 text-xs">
                                        <Award className="h-3 w-3 mr-1" />
                                        Best Seller
                                      </Badge>
                                    )}
                                    {metrics.badges.seller_amazon_choice && (
                                      <Badge className="bg-blue-100 text-blue-800 text-xs">
                                        <Star className="h-3 w-3 mr-1" />
                                        Amazon's Choice
                                      </Badge>
                                    )}
                                    {metrics.badges.seller_prime && (
                              ...
                              <div className="space-y-4">
                                <div className="flex items-center gap-2 mb-3">
                                  ...
                                  <h4 className="font-bold text-slate-800">Competitor</h4>
                                </div>
                                
                                <div className="flex gap-4">
                                  {comparison.competitor_product.product_photo && (
                                    <img 
                                      src={comparison.competitor_product.product_photo} 
                                      alt="Competitor"
                                  ...
                                  <div className="flex-1">
                                    <p className="font-semibold text-slate-900 text-sm leading-tight mb-2">
                                      {comparison.competitor_product.product_title}
                                    </p>
                                    <p className="text-xs text-slate-500">ASIN: {comparison.competitor_product.asin}</p>
                                  </div>
                                </div>
                                ...
                                <div className="space-y-2 bg-orange-50 p-3 rounded-lg">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-bold text-orange-700">
                                      ₹{metrics.price_comparison.competitor_price?.toFixed(2) || 'N/A'}
                                    </span>
                                  </div>
                                  <div className="flex items-center justify-between">
                                    <span className="text-xs text-slate-600">Rating:</span>
                                    <div className="flex items-center gap-1">
                                      <Star className="h-3 w-3 text-yellow-500 fill-yellow-500" />
                                      <span className="font-bold text-orange-700">
                                        {metrics.rating_comparison.competitor_rating?.toFixed(1) || 'N/A'}
                                      </span>
                                    </div>
                                  </div>
                                  <div className="flex items-center justify-between">
                                    <span className="text-xs text-slate-600">Reviews:</span>
                                    <span className="font-bold text-orange-700">
                                      {metrics.review_count_comparison.competitor_reviews?.toLocaleString() || 'N/A'}
                                    </span>
                                  </div>
                                  
                                  {/* Badges */}
                                  <div className="flex flex-wrap gap-1 pt-2">
                                    {metrics.badges.competitor_best_seller && (
                                      <Badge className="bg-orange-100 text-orange-800 text-xs">
                                        <Award className="h-3 w-3 mr-1" />
                                        Best Seller
                                      </Badge>
                                    )}
                                    {metrics.badges.competitor_amazon_choice && (
                                      <Badge className="bg-blue-100 text-blue-800 text-xs">
                                        <Star className="h-3 w-3 mr-1" />
                                        Amazon's Choice
                                      </Badge>
                                    )}
                                    {metrics.badges.competitor_prime && (
                                      <Badge className="bg-cyan-100 text-cyan-800 text-xs">
                                        Prime
                                      </Badge>
                                    )}
                            ...
                            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                              {/* Advantages */}
                              {metrics.competitive_advantages.length > 0 && (
                                <div className="bg-green-50 rounded-lg p-4 border-2 border-green-200">
                                  ...
                                  <ul className="space-y-2">
                                    {metrics.competitive_advantages.map((adv, i) => (
                              ...
                              {/* Disadvantages */}
                              {metrics.competitive_disadvantages.length > 0 && (
                                <div className="bg-red-50 rounded-lg p-4 border-2 border-red-200">
                                  ...
                                  <ul className="space-y-2">
                                    {metrics.competitive_disadvantages.map((dis, i) => (
                            ...
                            {/* Price Analysis */}
                            {metrics.price_comparison.difference !== null && (
                              <div className="mt-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4 border-2 border-purple-200">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-2">
                                    <DollarSign className="h-5 w-5 text-purple-600" />
                                    <span className="font-semibold text-slate-800">Price Analysis:</span>
                                  </div>
                                  <div className="text-right">
                                    {metrics.price_comparison.is_cheaper ? (
                                      <div className="flex items-center gap-2">
                                        <TrendingDown className="h-4 w-4 text-green-600" />
                                        <span className="font-bold text-green-600">
                                          {metrics.price_comparison.difference_percent?.toFixed(1)}% Cheaper
                                        </span>
                                      </div>
                                    ) : (
                                      <div className="flex items-center gap-2">
                                        ...
                                        <span className="font-bold text-red-600">
                                          {Math.abs(metrics.price_comparison.difference_percent || 0).toFixed(1)}% More Expensive
                                        </span>
                                      </div>
                                    )}
                                  </div>
